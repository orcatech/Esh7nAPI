...
public class CameraView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public CameraView (MyActivity activity, int cameraId, LayoutMode mode) {
    ...
    public void snapShot() {
        ...
        mCall = new Camera.PictureCallback() {
            public void onPictureTaken(byte[] data, Camera arg1) {
                ...
                if(mPictureSize.width >  3000)
                    options.inSampleSize = 4;
                else
                if(mPictureSize.width >  2000)
                ...
                if(isPortrait()){
                    ...
                    try{
                        ...
                        Toast.makeText(mActivity, "Not Enough Memory to Perform Operation!", Toast.LENGTH_LONG).show();
                        return;
                    }
                }


                double rW =  (double)bmp.getWidth() / (double)getWidth();
                double rH =  (double)bmp.getHeight() / (double)getHeight();


                int cropX = (int) (rW * CameraOverlay.CameraHole.left);
                int cropY = (int) (rH * CameraOverlay.CameraHole.top);
                int cropW = (int) (rW * CameraOverlay.CameraHole.width());
                int cropH = (int) (rH * CameraOverlay.CameraHole.height());

                bmp=Bitmap.createBitmap(bmp, cropX,cropY,cropW, cropH);
                ByteArrayOutputStream bos = new ByteArrayOutputStream();

                bmp.compress(CompressFormat.JPEG, 100, bos);
                byte[] imageToSend = bos.toByteArray();
                mActivity._esh7n.DetectNumbers(imageToSend, "");
